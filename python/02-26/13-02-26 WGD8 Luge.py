...
Given an array of five numbers, each representing the time (in seconds) it took a luger to complete a segment of a track,
...
Return "The luger's fastest speed was X m/s on segment Y.". Where X is the fastest speed, rounded to two decimal places,
...
def get_fastest_speed(times: list[float]) -> str:
    ...
    speeds: list[float] = [(segments[i] / time) for i, time in enumerate(times, start=1)]
    max_speed: float = max(speeds)

    return f"The luger's fastest speed was {max_speed:.2f} m/s on segment {speeds.index(max_speed) + 1}."
