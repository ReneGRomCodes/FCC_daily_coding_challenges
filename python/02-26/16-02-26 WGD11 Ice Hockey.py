...
def get_semifinal_matchups(teams):
    ...
    for item in teams:
        # Indexing instead of splitting into a list here to keep the code a bit more concise.
        team, wins, ot_wins, ot_loss = item[:3], int(item[5]), int(item[7]), int(item[9])
        points.append((team, wins * 3 + ot_wins * 2 + ot_loss * 1))

    points.sort(key=lambda x: x[1], reverse=True)

    return f"The semi-final games will be {points[0][0]} vs {points[3][0]} and {points[1][0]} vs {points[2][0]}."
