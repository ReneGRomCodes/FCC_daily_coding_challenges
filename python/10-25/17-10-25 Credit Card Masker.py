...
The string will contain four sets of four digits (0-9), with all sets being separated by a single space, or a single
hyphen (-).
...
def mask(card: str) -> str:
    ...
    for digit_set in card_list:
        if digit_set is card_list[-1]:
            masked_card += digit_set
        else:
            masked_card += replacement
