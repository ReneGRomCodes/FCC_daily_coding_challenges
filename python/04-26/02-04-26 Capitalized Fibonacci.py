...
def get_fibonacci_set(limit: int) -> set[int]:
    fib_set: set[int] = set()
    a, b = 0, 1
...
def capitalize_fibonacci(s: str) -> str:
    fibonacci: set[int] = get_fibonacci_set(len(s))
    new_s: str = ""

    for index, char in enumerate(s.lower()):
        if index in fibonacci:
            new_s += char.upper()
        else:
            new_s += char
...
print(capitalize_fibonacci("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin pulvinar ex nibh, vel ullamcorper "
