...
function getJetLagHours(departureCity, arrivalCity, flightDuration, direction) {
    ...
    const tzDifference = Math.abs(utcOffset[departureCity.toLowerCase()] - utcOffset[arrivalCity.toLowerCase()]);
    const directionMultiplier = direction.toLowerCase() === "east" ? 1.5 : 1.0;
    const jetLagHours = tzDifference + (flightDuration * 0.1) * directionMultiplier;

    return Number(jetLagHours.toFixed(1));
}
