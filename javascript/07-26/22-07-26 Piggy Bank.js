...
function piggyBank(coins) {
    ...
    if ("pennies" in coins) { value += coins["pennies"] * 0.01 }
    if ("nickels" in coins) { value += coins["nickels"] * 0.05 }
    if ("dimes" in coins) { value += coins["dimes"] * 0.1 }
    if ("quarters" in coins) { value += coins["quarters"] * 0.25 }
