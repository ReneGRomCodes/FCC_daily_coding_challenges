"""
Pairwise
Given an array of integers and a target number, find all pairs of elements in the array whose values add up to the target
...
def pairwise(arr: list[int], target: int) -> int:
    ...
    for i, n in enumerate(arr):
        for j, m in enumerate(arr[i+1:], start=i):
            if n + m == target:
