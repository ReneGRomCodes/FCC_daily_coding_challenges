...
def wise_speak(sentence: str) -> str | None:
    indicator_words: tuple[str, ...] = ("have", "must", "are", "will", "can")

    for word in indicator_words:
        if word in sentence:
            word_index: int = sentence.index(word)

            return f"{sentence[word_index + len(word):-1].lstrip().capitalize()}, {sentence[:word_index + len(word)].lower()}{sentence[-1]}"
