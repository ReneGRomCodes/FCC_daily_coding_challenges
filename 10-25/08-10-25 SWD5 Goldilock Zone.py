...
For the fifth day of Space Week, you will calculate the "Goldilocks zone" of a star - the region around a star where
...
def goldilocks_zone(mass) -> list[float]:
    lum: float = mass**3.5
    output_list: list[float] = [round(math.sqrt(lum) * 0.95, 2), round(math.sqrt(lum) * 1.37, 2)]
